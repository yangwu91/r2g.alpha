...
"""
# Yang Wu modified it based on Commit ba613e2 for specific purposes.

# from __future__ import print_function

from r2g.Bio._py3k import StringIO
from r2g.Bio._py3k import _as_string, _as_bytes
from r2g.Bio._py3k import urlopen as _urlopen
from r2g.Bio._py3k import urlencode as _urlencode
from r2g.Bio._py3k import Request as _Request
...
def _add_organism(organisms):
    organisms = organisms.strip().split(',')
    org_parameters = [('EQ_MENU', organisms[0]), ]
    if len(organisms) > 1:
        for i in range(1, len(organisms)):
            org_parameters.append(('EQ_MENU{}'.format(i), organisms[i]))
...
def qblast(program, database, sequence, url_base=NCBI_BLAST_URL,
           ...
           template_type=None, template_length=None, organisms=None,
           num_threads=None, max_num_seq=500, timeout=1200
           ):
    ...
    https://blast.ncbi.nlm.nih.gov/Blast.cgi?PAGE_TYPE=BlastDocs&DOC_TYPE
    =CloudBlast
    ...
    Some useful parameters:
     ...
     - matrix_name    Specify an alt. matrix (PAM30, PAM70, BLOSUM80,
                      BLOSUM45).
     - filter         "none" turns off filtering.  Default no filtering
     - format_type    "HTML", "Text", "ASN.1", or "XML".  Def. "XML".
     - entrez_query   Entrez query to limit Blast search
     - hitlist_size   Number of hits to return. Default 50
     - megablast      TRUE/FALSE whether to use MEga BLAST algorithm (blastn
                      only)
     - service        plain, psi, phi, rpsblast, megablast (lower case)
     - organisms      Specify an organism while using nr/nt database, or an SRA
                      accession for SRA, EST and other databases. The organism
                      also could be specified by "entrez_query". For example:
                      "entrez_query='rat [ORGN]'" is identical to
                      "organism='rat (taxid:10114)'".
     - num_threads    Supported only on the cloud.
     - max_num_seq    Maximum number of aligned sequences to retrieve (default:
                      1000, the actual number of alignments may be greater than
                      this).
    ...
    if program not in programs:
        raise ValueError(
            "Program specified is {}. Expected one of {}".format(
                program, ", ".join(programs)
            )
        )
    if url_base == "https://blast.ncbi.nlm.nih.gov/Blast.cgi" and \
            num_threads is not None:
        raise ValueError(
            'The "num_threads" option cannot be used on NCBI public servers.'
        )

    # Format the "Put" command, which sends search requests to qblast.
    # Parameters taken from http://www.ncbi.nlm.nih.gov/BLAST/Doc/node5.html on
    # 9 July 2007
    # Additional parameters are taken from
    # http://www.ncbi.nlm.nih.gov/BLAST/Doc/node9.html on 8 Oct 2010
    # To perform a PSI-BLAST or PHI-BLAST search the service ("Put" and "Get"
    # commands) must be specified (e.g. psi_blast = NCBIWWW.qblast("blastp",
    # "refseq_protein", input_sequence, service="psi"))
    # The aforementioned webpages were not valid on Jan 19, 2018. Current API
    # webpage is: https://ncbi.github.io/blast-cloud/dev/api.html
    parameters = [
    ...
    if organisms is not None:
        parameters += _add_organism(organisms)
    ...
    # Parameters taken from http://www.ncbi.nlm.nih.gov/BLAST/Doc/node6.html
    # on 9 July 2007
    # The aforementioned webpage was not valid on Jan 19, 2018. Current API
    # webpage is: https://ncbi.github.io/blast-cloud/dev/api.html
    ...
    while True:
        ...
        if current - previous > timeout:
            raise ValueError("{}s timeout.".format(timeout))
